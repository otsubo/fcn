...
def overlay_color_on_mono(img_color, img_mono, alpha=0.3):
    ...
    dst = skimage.util.img_as_ubyte(dst)
    return dst
...
def resize(image, height=None, width=None, fy=None, fx=None,
           max_size=None, size=None, interpolation=cv2.INTER_LINEAR):
    if max_size is not None:
        warnings.warn('max_size will be renamed to size.')
        assert size is None
        size = max_size
